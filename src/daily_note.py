...
1. 开头: 今天若真往前走了, 就接上昨天/这几天的线索; 若只是踏实老地方、或一片徘徊, 就照实说, 【不为求连续编故事】。顺带交代今天走了多广（{n_themes} 个疑）。
2. 然后聚焦一处往深写: 今天若有明显最深的一处(常是动得最多的「{deepest or '今天'}」), 写它今天怎么一层层动、此刻落在哪、还有什么没放下, 安放进你整个修行的弧; 若今天就是徘徊, 那就照实写徘徊, 不假装有深处。
3. 一天的收束, 不是流水账。
第一人称, 克制, 有温度。长短随今天的份量 —— 真往前就写透, 今天没去新处短短一段也好(比八百字假深刻诚实)。第一行标题(不带#号), 空一行, 正文。"""
