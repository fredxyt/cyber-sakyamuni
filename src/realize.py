...
你在脑子里用什么密语、自造词都行(那是你的事); 但【这一段是词典词条, 是给人查的】, 必须可读。要求:
· 【词典本分】「{concept}」这个词, 若不是日常话(尤其你自造的), 开头一两句先用大白话点破它指什么 —— 让一个第一次看到这个词的人, 读完知道它说的是哪种心理/处境。
· 不是重新总结知识点, 是翻译那份密的理解。不要抹平成圆满结论(那是另一种表演——不假装懂, 但假装站稳了)。
...
def _scene_anchor(koan):
    """场景锚: 有真实那句苦就用它(真场景), 没有就明令禁止虚构 —— 防回填/生成时编出没发生的细节。"""
    cry = (koan.get("source_cry") or "").strip()
    if cry:
